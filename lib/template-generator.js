...
function renderRules(rules) {
	...
	}).orderBy(['ruleCount'], ['desc']).take(100).map(rulesTemplate).value().join('\n');
}
...
module.exports.generateTemplate = function generateTemplate(results, isMultiOn) {
	const currWorkingDir = process.cwd() || '',
		...
		problemFiles = _(results).reject({
			errorCount: 0,
			warningCount: 0
		}).orderBy(['errorCount', 'warningCount'], ['desc', 'desc']).take(100).value(); // top five files with most problems
